      ...
      <div className="mb-6 flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-1.5">
          ...
          {exercices.map((e) => (
            <Link
              key={e.exercice}
              href={`/representants/partis?exercice=${e.exercice}${q ? `&q=${q}` : ""}${tri !== "produits" ? `&tri=${tri}` : ""}`}
        ...
        <div className="flex items-center gap-1.5">
          ...
          {tris.map((t) => (
            <Link
              key={t.key}
              href={`/representants/partis?exercice=${exercice}${q ? `&q=${q}` : ""}${t.key !== "produits" ? `&tri=${t.key}` : ""}`}
              className={`rounded-md px-2.5 py-1 text-xs transition-colors ${
                tri === t.key
                  ? "bg-teal/10 text-teal"
                  : "text-bureau-400 hover:bg-bureau-800"
      ...
      <div className="grid gap-2">
        {partis.map((p) => {
          ...
          return (
            <Link
              key={p.id}
              href={`/representants/partis/${p.id}`}
