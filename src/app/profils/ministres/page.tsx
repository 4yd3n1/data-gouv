...
const GOV_SLUGS: Record<string, string> = {
  borne: "Gouvernement \u00c9lisabeth Borne",
  attal: "Gouvernement Gabriel Attal",
  barnier: "Gouvernement Michel Barnier",
  lecornu: "Gouvernement S\u00e9bastien Lecornu",
};
...
export default async function GouvernementPage({
  ...
  const { gouvernement: govFilter } = await searchParams;
  const govName = govFilter ? GOV_SLUGS[govFilter] : undefined;
  const isHistorical = !!govName;

  const mandateWhere = govName
    ? { gouvernement: govName }
    : { dateFin: null };

  const [personnalites, distinctGovs] = await Promise.all([
    ...
    prisma.mandatGouvernemental.findMany({
      select: { gouvernement: true, dateDebut: true, dateFin: true },
      distinct: ["gouvernement"],
  ...
  const gouvernement = govName ?? sorted[0]?.mandats[0]?.gouvernement ?? "Gouvernement";
  const total = sorted.length;
  const docRef = `GOV-FR-${new Date().getFullYear()}-${String(total).padStart(3, "0")}`;

  // Build historical government links (exclude Bayrou, exclude current)
  const govLinks = Object.entries(GOV_SLUGS)
    .filter(([, name]) => distinctGovs.some(g => g.gouvernement === name))
    .map(([slug, name]) => {
      const record = distinctGovs.find(g => g.gouvernement === name);
      return { slug, name, dateDebut: record?.dateDebut, dateFin: record?.dateFin };
    });
  ...
  return (
    <div className="min-h-screen bg-bureau-950 text-bureau-100">
      ...
      <div className="mx-auto max-w-5xl px-5 pb-32 pt-10 sm:px-8">
        ...
        {/* ── Historical government selector ── */}
        {govLinks.length > 1 && (
          <div className="mt-16">
            <h2 className="mb-4 text-xs font-semibold uppercase tracking-[0.15em] text-bureau-500">
              {isHistorical ? "Autres gouvernements" : "Gouvernements pr\u00e9c\u00e9dents"}
            </h2>
            <div className="flex flex-wrap gap-2">
              {!isHistorical ? null : (
                <Link
                  href="/gouvernement"
                  className="rounded-md border border-teal/20 bg-teal/5 px-3 py-1.5 font-mono text-[10px] text-teal transition-colors hover:bg-teal/10"
                >
                  Gouvernement actuel
                </Link>
              )}
              {govLinks.map(gl => {
                const isActive = gl.name === gouvernement;
                if (isActive) return null;
                return (
                  <Link
                    key={gl.slug}
                    href={`/gouvernement?gouvernement=${gl.slug}`}
                    className="rounded-md border border-bureau-700/20 bg-bureau-800/20 px-3 py-1.5 font-mono text-[10px] text-bureau-400 transition-colors hover:border-bureau-600/40 hover:text-bureau-200"
                  >
                    {gl.name}
                    {gl.dateFin && (
                      <span className="ml-1.5 text-bureau-600">
                        ({fmtDate(gl.dateDebut)} &ndash; {fmtDate(gl.dateFin)})
                      </span>
                    )}
                  </Link>
                );
